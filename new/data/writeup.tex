...
\end{equation}
Using relation~\eqref{stage_eqn_first}
\begin{equation}
  \mathbf{R}\left(t_{k,i}, \mathbf{q}_k + h \sum_{j=1}^s
  a_{ij}\mathbf{\dot{q}}_{k,j}, \mathbf{\dot{q}}_{k,i}\right) = 0
\end{equation}
In Eq.~\ref{stage_eqn_first} we solve for $\mathbf{\dot{q}}_{k,i}$
values. If one uses DIRK, Eq~\ref{stage_eqn_desc} can be solved
